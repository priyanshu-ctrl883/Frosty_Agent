...
import { updatePassword, waitForPasswordRecovery, signOut, resetPasswordWithToken } from "@/lib/session";
...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const recovered = await waitForPasswordRecovery();
        if (!cancelled) setHasSession(recovered);
      } catch {
  ...
  function getPasswordStrength(p: string) {
    ...
    if (score < 2) return { label: "Weak", color: "#e11d48", score };
    if (score < 4) return { label: "Good", color: "#d97706", score };
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      if (iamToken) {
        await resetPasswordWithToken(iamToken, password);
      } else {
  ...
  if (verifyingSession) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-white text-slate-800">
        <div className="flex flex-col items-center gap-3">
          <div className="w-12 h-12 rounded-[18px] bg-[#0092A2] text-white flex items-center justify-center shadow-lg animate-pulse">
            <FrostrekLogo size={28} color="#ffffff" />
          </div>
          <p className="text-sm font-semibold text-slate-500">Verifying secure recovery link...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen w-full bg-white text-slate-900 font-sans selection:bg-[#0092A2]/20 flex flex-col justify-between relative overflow-x-hidden">
      ...
      <div className="w-full max-w-[1340px] mx-auto px-5 sm:px-8 lg:px-10 py-6 sm:py-8 flex-1 flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-12 relative z-10 my-auto">
          ...
          <div className="mt-8 relative pt-3 pb-2 max-w-lg">
            ...
            >
              ...
              <motion.path
                ...
                strokeLinecap="round"
                opacity="0.9"
                markerEnd="url(#reset-sketch-arrow)"
        ...
        {/* ── RIGHT COLUMN: Elevated White Reset Form Card ── */}
        <div className="w-full lg:w-[42%] flex flex-col items-center justify-center">
          <div className="w-full max-w-[420px] bg-white rounded-[32px] p-8 sm:p-10 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.07),0_8px_30px_-6px_rgba(0,146,162,0.05)] border border-slate-100/90 transition-all duration-300">
            ...
            {success ? (
              <div className="space-y-4 text-center">
                <div className="p-4 bg-[#EEF6F8] border border-[#BEE7E9] rounded-2xl flex items-start gap-3 text-xs text-slate-800 text-left">
                  <CheckCircle2 className="w-5 h-5 text-[#0092A2] shrink-0 mt-0.5" />
                  <div>
                    <p className="font-bold text-[#0092A2] text-sm">Password updated successfully</p>
                    <p className="mt-1 text-slate-600 leading-relaxed">
                      Your credentials were saved securely. You can now log into your merchant workspace.
                    </p>
                  </div>
                </div>
                <Link
              ...
              <div className="space-y-4 text-center">
                <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-start gap-3 text-xs text-amber-800 text-left">
                  <ShieldAlert className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
                  <div>
                    <p className="font-bold text-amber-900 text-sm">Invalid or Expired Link</p>
                    <p className="mt-1 text-amber-700 leading-relaxed">
                      This recovery link has expired or has already been used. Please request a new recovery link.
                    </p>
                  </div>
                </div>
                <Link
              ...
              <form className="space-y-3.5" onSubmit={onSubmit}>
                ...
                {!SUPABASE_CONFIGURED && (
                  <p className="text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-md p-2">
                    Password update is not configured for this build.
                  </p>
                )}
            ...
            <div className="mt-6 text-center text-xs text-slate-500 font-medium">
              ...
              <Link href="/login" className="text-[#0092A2] font-bold hover:underline">
                Sign in
              </Link>
            </div>
          </div>
