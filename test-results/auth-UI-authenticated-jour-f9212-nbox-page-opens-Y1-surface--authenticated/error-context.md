...
- Name: auth.spec.ts >> UI authenticated journeys >> UI-TC-0011 inbox page opens (Y1 surface)
...
Locator: getByText(/waiting|inbox|claim|the agent is handling/i).first()
Expected: visible
Timeout: 30000ms
...
Call log:
  ...
  - waiting for getByText(/waiting|inbox|claim|the agent is handling/i).first()
